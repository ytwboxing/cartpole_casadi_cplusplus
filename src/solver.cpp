...
Solver::Solver(const cart_pole_model& _model, const constraint_value& _constraint)
 :model(_model), solverState(SolverState::NOT_INITIALIZED), solution(nullptr)
{
...
void Solver::setParametersValue(const State& initialState, const State& finalState)
{
    opti.set_value(initialStateParameters, initialState.state);
    opti.set_value(finalStateParameters, finalState.state);
}

bool Solver::solve(const State& initialState, const State& finalState)
{
...
void Solver::getSolution(casadi::DM& state, casadi::DM& control){
    state = solution -> value(X);
